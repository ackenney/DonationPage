...
const phoneValidationPattern = /^[\+]?[0-9]{0,3}\W?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/;
const numberValidationPattern = /^\d{5}$/;
...
export const validatePhone = (number: string): boolean => phoneValidationPattern.test(number);
