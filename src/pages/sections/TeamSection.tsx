...
export default function TeamSection() {
  return (
    <Section id="team" className="bg-sky-800">
      <div className="w-full lg:max-w-xl lg:mx-auto bg-[url('/images/philadelphia-skyline-1219756.jpg')] bg-cover bg-no-repeat bg-top bg-fixed">
            ...
            {teamMembers.map((member, index) => (
              <div key={index} className="flex flex-col  bg-white rounded-xl items-center text-center">
                <img
                  src={member.imageUrl}
                  alt={member.name}
                  className="w-40 h-40 TextColor1 object-cover rounded-xl"
                />
                <h3 className="text-TextColor2 font-bold mt-2">{member.title}</h3>
                <p className="text-TextColor1 mt-1">{member.name}</p>
                <p className=" mt-2"></p>
              </div>
            ))}
          </div>
